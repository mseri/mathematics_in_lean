...
-- Set (Set X): set of subsets of sets of X
...
-- use sets to get a better idea, but beware that the
-- inclusion directions are kind of flipped
example (f : X → Y) (F : Set X) (G : Set Y) :
    f '' F ⊆ G ↔ F ⊆ f ⁻¹' G := by
...
example (f : X → Y) (F : Filter X) (G : Filter Y) :
  ...
  exact?
  -- every element of G has pre-image in F
...
example (u : ℕ → ℝ) (x : ℝ) : Prop :=
  Tendsto u atTop (𝓝 x)
  -- u tends to neighbourhood of x as n tends to atTop
  -- for any nbd of x, the preimage of that under u,
  -- lies in the filter in X
  -- that is, any nbhd of x contains eventually infinitely many
  -- growing natural numbers
...
example {X Y Z : Type _} {F : Filter X} {G : Filter Y} {H : Filter Z}
    ...
    Tendsto (g ∘ f) F H := by
  apply Iff.mpr (Tendsto_iff (g ∘ f) F H)
  -- unfold Tendsto at hf -- to check what's in hf definition
  intro S hs
  exact hf (hg hs)
  done
...
It is defined to be `{ x | P x } ∈ F`. <- The set of P x such that
P holds is an element of F.
...
example {x : X} {s : Set X} (h : s ∈ 𝓝 x) : x ∈ s := by
  -- first we use the characterization in therms of subsets
  rw [mem_nhds_iff] at h
  -- we can then extract a witness and its properties
  rcases h with ⟨t, hts, _ht, hxt⟩
  -- and the hypoteses there give the proof
  exact hts hxt
  done
...
-- Finset: finite subset
-- ⋃ i ∈ t, U i: union of all Us in t
...
-- Intersection of nbhds of x with the filter is not the empty set
example (F : Filter X) :
